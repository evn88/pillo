...
vi.mock('@/components/screen-actions', () => ({ ScreenActions: () => null, supportsTabAccessory: true }));
...
describe('Плановый приём', () => {
  it('отдельное действие открывает ручной ввод даже при ожидающем расписании', async () => {
    ...
    expect(screen!.root.findByType(ScreenActions).props.inlineFallback).toBe(false);
  ...
  it('действие свайпа открывает изменение дозы без регистрации приёма', async () => {
    context.current = createContext();
    let screen: ReturnType<typeof create>;
    await act(async () => { screen = create(<TodayScreen isLargeText={false} />); });
    await act(async () => {
