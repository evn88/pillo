...
| A21 | 5.3, 5.6 | Явные labels/actions, единая keyboard avoidance, native scroll insets и layout от контейнера; iOS Simulator Release smoke при максимальном Dynamic Type, portrait/landscape и accessibility tree; `swipeable-card.test.tsx` | VoiceOver/TalkBack, keyboard, iPad split view и Android UI: 5.3, 5.6 |
...
| A23 | 6.1–6.2 | `app.json` и сгенерированный Android manifest отключают backup; `PilloDataPrivacy` исключает iOS SQLite/WAL/SHM при открытии и после записи, а iOS Simulator Runtime подтверждает xattr на всех трёх файлах; clear-data и delivered-notification retry покрыты `pillo-controller.test.ts` | Android/iOS backup/transfer restore на физическом устройстве: 6.1 |
...
| A26 | 0.3–0.4, 7.1–7.5 | 71 автоматический тест, lint, exports, unsigned iOS Release compile-only и Simulator install/launch; `native-build-matrix.md` | Подпись, Android build, v1 migration и physical-device matrix: 7.2, 7.3 |
