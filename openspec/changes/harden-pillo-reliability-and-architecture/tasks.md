...
- [x] 2.4 Сделать ошибки полей доступными, pending безопасным, quick intake с видимой дозой; ввод не исчезает при отказе. Плановый приём поддерживает отдельную атомарную команду фактической дозы, а расписание, ручная запись и подтверждение используют общий выбор ¼, ½, 1, 2 и произвольного количества.
...
- [x] 4.4 Добавить статус requested/authorized/exact-capability/channel/sync/coverage, обработку denied/provisional/revoked, возврата из системных настроек. Настроить необходимые Android permissions только по проверенному контракту. В исправном состоянии ручная синхронизация скрыта; retry показывается после ошибки, запрос разрешения — только пока ОС допускает его, а окончательный запрет ведёт в системные настройки. Foreground refresh автоматически повторяет сверку. Доказательство: `evidence/notification-handling.md`; точность Android остаётся честно `unknown` до native spike 4.1.
...
- [ ] 5.3 Исправить labels, accessibility actions, nested presses, safe area, keyboard avoidance, размеры от контейнера, большой шрифт и split view. Выполнены статические исправления labels/actions, scroll insets, keyboard avoidance и отказ от window-width/fixed tab offset; действия экранов перенесены из absolute footer в системный iOS 26 BottomAccessory; при крупном тексте и на остальных ОС используется scroll flow. Floating tab bar сохраняет нижний content clearance. При `fontScale >= 1.4` header, карточки, history rows, modal headings, actions и theme options переходят в безопасную вертикальную компоновку; ручной приём использует компактное поле препарата и отдельный нативный picker с поиском и виртуализированным списком, включая тест на 120 записей. Список препаратов переведён на FlatList: одна колонка на iPhone, две на iPad и снова одна при крупном тексте; независимые действия карточки имеют равные области, SF Symbols и полные accessibility labels. Форма препарата получила закреплённые действия, секции, нативный menu Picker с произвольным вариантом и одноколоночные строки запаса; iOS ScrollView сам учитывает клавиатуру без двойного смещения листа. Ожидающий приём показывает дозу прямо в основном действии и имеет отдельную доступную кнопку её изменения; скрытый жест не является единственным входом. Общий `DoseInput` применяется в расписании и обоих способах отметки. iOS 26.5 Simulator Release проверен при `accessibility-extra-extra-extra-large` в portrait/landscape. Regression: `swipeable-card.test.tsx`, `manual-intake-sheet.test.tsx`, `medications-screen.test.tsx`, `medication-form.test.tsx`, `dose-input.test.tsx`, `schedule-form.test.tsx`, `scheduled-intake-sheet.test.tsx`, `today-screen.test.tsx`. VoiceOver/TalkBack, iPad split view и Android остаются device-приёмкой.
...
- [ ] 5.6 Проверить четыре раздела, создание/редактирование/ошибки форм, VoiceOver/TalkBack, Android back, rotation, small phone/tablet, light/dark и старый iOS UI fallback. iPhone 17 Pro iOS 26.5 Simulator Release: все четыре tabs визуально проверены; основные и последние прокручиваемые действия доступны над floating tab bar, ручная отметка открывается, максимальный Dynamic Type и rotation проверены. UX-этап: розовая public-палитра, закреплённые кнопки форм, SwiftUI theme/date/time pickers, SF Symbols; открытие форм и истории проверено без записи данных. Экран препаратов повторно проверен после list-layout: две карточки и их парные действия не перекрываются BottomAccessory; карточка и «Отметить» открывают разные формы, обе закрыты без сохранения. Форма создания препарата проверена отдельно: системное меню показывает семь частых форм и «Другая…», произвольный вариант раскрывает ввод, числовые строки прокручиваются, верхние действия остаются доступны; пустое название показывает ошибку без записи. Форма расписания и ручной приём визуально проверены с дробными preset-кнопками и свободным вводом; редактор фактической плановой дозы проверен автоматически, но не вызван на сохранённых данных симулятора. Полная проверка успешного сохранения, tablet, Android и physical-device matrix остаются.
