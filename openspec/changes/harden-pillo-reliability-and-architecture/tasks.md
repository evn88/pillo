...
- [ ] 5.3 Исправить labels, accessibility actions, nested presses, safe area, keyboard avoidance, размеры от контейнера, большой шрифт и split view.
...
- [ ] 7.2 Выполнить exports iOS/Android, затем настоящие native release-сборки и установку. Проверить обновление поверх v1 с историей, rollback/recovery и повторный запуск.
...
- [ ] 7.4 Проверить трассировку каждого Axx → задача → тест/доказательство, не закрывать подтверждённые проблемы только переносом файлов.
- [ ] 7.5 Выполнить `openspec validate harden-pillo-reliability-and-architecture --strict` и `git diff --check`; синхронизировать документы с фактическими результатами. Архивировать изменение только после приёмки, отдельно от публикации приложения.
